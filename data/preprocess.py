...
df_adm = pd.read_csv('/Users/jinglv/Desktop/second_year/NNDL/clinicalBERT-master/ADMISSIONS.csv')
...
df_adm['NEXT_ADMISSION_LOCATION'] = df_adm.groupby('SUBJECT_ID').ADMISSION_LOCATION.shift(-1)
df_adm['NEXT_DISCHARGE_LOCATION'] = df_adm.groupby('SUBJECT_ID').DISCHARGE_LOCATION.shift(-1)


rows = df_adm.NEXT_ADMISSION_TYPE == 'ELECTIVE'
df_adm.loc[rows,'NEXT_ADMITTIME'] = pd.NaT
df_adm.loc[rows,'NEXT_ADMISSION_TYPE'] = np.NaN
df_adm.loc[rows,'NEXT_ADMISSION_LOCATION'] = np.NaN
df_adm.loc[rows,'NEXT_DISCHARGE_LOCATION']  =  np.NaN
...
df_adm[['NEXT_ADMITTIME','NEXT_ADMISSION_TYPE' ,"NEXT_ADMISSION_LOCATION","NEXT_DISCHARGE_LOCATION"]] = df_adm.groupby(['SUBJECT_ID'])[['NEXT_ADMITTIME','NEXT_ADMISSION_TYPE'
                                                                                   ,"NEXT_ADMISSION_LOCATION","NEXT_DISCHARGE_LOCATION"]].fillna(method = 'bfill')
...
df_notes = pd.read_csv('/Users/jinglv/Desktop/second_year/NNDL/clinicalBERT-master/NOTEEVENTS.csv',engine='python',error_bad_lines=False)
...
df_adm_notes = pd.merge(df_adm[['SUBJECT_ID','HADM_ID','ADMITTIME','DISCHTIME','DAYS_NEXT_ADMIT','NEXT_ADMITTIME','ADMISSION_TYPE','DEATHTIME','OUTPUT_LABEL','DURATION',
                                'NEXT_ADMISSION_TYPE','NEXT_ADMISSION_LOCATION','NEXT_DISCHARGE_LOCATION']],
...
df_adm.head(1000).to_csv('admission_sample.csv')
df_notes.head(1000).to_csv('note_sample.csv')
df_adm_notes.head(1000).to_csv('combine_sample.csv')
df_adm.to_csv('admission.csv')
df_notes.to_csv('note.csv')
df_adm_notes.to_csv('combine.csv')
